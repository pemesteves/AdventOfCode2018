...
int main(){
	...
	for(it1 = m1.begin(); it1 != m1.end(); it1++){
		unsigned internalCounter = 0;
		unsigned internalMinute = 0;
		unsigned counterNumTimes = 0;

		for(size_t i = 0; i < (*it1).second.size(); i++){

			if((*it1).second[i] >= 1){
				internalCounter++;
				if ((*it1).second[i] >= counterNumTimes) {
					counterNumTimes = (*it1).second[i];
					internalMinute = i;
				}
			}
		}

		if (numMinutesSleeping <= internalCounter){
			minute = internalMinute;
			numMinutesSleeping = internalCounter;
			id = (*it1).first; 
		}
	}
